...
Diretivas do compilador são apenas comandos ao montador e não fazem parte do conjunto de instruções de nenhum processador, mas são elas que permitem a alocação de espaço para a declaração de variáveis " .byte, .word ", definição de escopo " .glob1 ", além várias outras funções de gerenciamento. 
...
Há também outras instruções que o simulador emitiu alertas, como por exemplo sobre \textit{.align} não poder estar dentro de um segmento de texto, portanto a diretiva \textit{.text} foi colocada após o \textit{label} \textit{\$LC0} e as diretivas \textit{.align} dentro de subrotinas foram removidas.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{hilo.png}
	\caption{Instruções equivalentes dos construtores \%hi() e \%lo().}
	\label{fig:hilo}
\end{figure}

Os construtores \%hi() e \%lo() também não estão presentes em todos montadores MIPS, podendo ser substituídos como mostra a Figura~\ref{fig:hilo}. Portanto as seguintes instruções foram trocadas :
